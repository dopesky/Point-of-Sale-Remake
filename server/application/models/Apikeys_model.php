...
class Apikeys_model extends MY_Model {
	...
	function add_api_key($owner_id, $scope, $apikey) {
		...
		if ($new_id) $this->api_activity_logs->add_log($new_id, "Registered new Autogenerated API Key through the API Keys Interface. API Key ID: $new_id");
		$this->db->trans_complete();
		return $this->db->trans_status() && $new_id ? $new_id : false;
	}
	...
	function update_api_key($apikey_id, $data) {
		...
		if ($result) $this->api_activity_logs->add_log($apikey['owner_id'], "Updated Scope of API Key with ID $apikey_id from {$apikey['scope']} to {$data['scope']} through the API Keys Interface.");
		$this->db->trans_complete();
